...
import type { ICertificateContentFullNameForm } from '#achievement/config/types';
import ContentTextBase from './ContentTextBase.vue';

const props = defineProps<{
  contentItem: ICertificateContentFullNameForm;
  index: number;
  isExpanded?: boolean;
}>();
...
const emit = defineEmits<{
  'delete': [index: number];
  'update:contentItem': [value: ICertificateContentFullNameForm];
  'headerClick': [];
}>();

const updateValue = (value: string | undefined) => {
  const updatedItem: ICertificateContentFullNameForm = {
    ...props.contentItem,
