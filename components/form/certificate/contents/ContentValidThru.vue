...
import type { ICertificateContentValidThruForm } from '#achievement/config/types';
import ContentTextBase from './ContentTextBase.vue';

const props = defineProps<{
  contentItem: ICertificateContentValidThruForm;
  index: number;
  isExpanded?: boolean;
}>();
...
const emit = defineEmits<{
  'delete': [index: number];
  'update:contentItem': [value: ICertificateContentValidThruForm];
  'headerClick': [];
}>();

const updateValue = (value: string | undefined) => {
  const updatedItem: ICertificateContentValidThruForm = {
    ...props.contentItem,
    value: value || '{{ valid_thru }}',
  };
